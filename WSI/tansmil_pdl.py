...
from models.transmil_pdl import TransMIL
...
def main():
    ...
    parser.add_argument('--save_dir', default='./trained_models/tcga_ss_transmil', type=str, help='the directory used to save all the output')
    args = parser.parse_args()
    gpu_ids = tuple(args.gpu_index)
    os.environ['CUDA_VISIBLE_DEVICES']=','.join(str(x) for x in gpu_ids)
    
    maybe_mkdir_p(join(args.save_dir, "transmil_pdl"))
    args.save_dir = make_dirs(join(args.save_dir, "transmil_pdl"))
    ...
    if args.optim == 'RAdam':
        base_optimizer = RAdam(milnet.parameters(), lr=args.lr, betas=(0.5, 0.9), weight_decay=args.weight_decay)
    ...
    dropout_schedule = LinearScheduler(milnet,start_value=0,stop_value=0.45,nr_steps=args.num_epochs)
    best_score = 0

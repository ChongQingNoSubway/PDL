...
from models.classic_network import AttnMIL_pdl,Pdrop_GatedAttention
...
def epoch_train(bag_ins_list, optimizer, criterion, milnet, args):
    ...
    for i, data in enumerate(bag_ins_list):
        ...
        bag_prediction, _= milnet(data_tensor) # n X L
...
def main():
    ...
    parser = argparse.ArgumentParser(description='classifier MIL datasets')
    parser.add_argument('--num_classes', default=1, type=int, help='Number of output classes [2]')
    parser.add_argument('--datasets', default='fox', type=str, help=' musk1, musk2, elephant, fox, tiger [musk1]')
    ...
    parser.add_argument('--drop_p', default=0.45, type=float, help='Bag classifier dropout rate [0]')
    ...
    for k in range(0, args.cv_fold):
        ...
        if args.model == "attn":
            milnet = AttnMIL_pdl(args.num_feats,args.num_classes,args.drop_p).cuda()
        elif args.model == "gate":
            milnet = Pdrop_GatedAttention(args.num_feats,args.num_classes,args.drop_p).cuda()
        elif args.model == 'dsmil':
            pdrop = False
            args.mDim = 32
            L = 64
            args.drop_p = 0.45
            i_classifier = mil.FCLayer(in_size=L, out_size=args.num_classes).cuda()
            b_classifier = mil.BClassifier(input_size=L, mDim =args.mDim, output_class=args.num_classes).cuda()
            milnet = mil.MILNet(i_classifier, b_classifier,pdrop,args.num_feats).cuda()

        
        dropout_schedule = LinearScheduler(milnet,start_value=0,stop_value=args.drop_p,nr_steps=args.num_epoch)
